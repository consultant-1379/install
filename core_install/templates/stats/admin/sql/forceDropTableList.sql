if (object_id('dba.forceDropTableList') is not   null)   then
    drop procedure dba.forceDropTableList ;

    if (object_id('dba.forceDropTableList') is not null) then
        message '<<< FAILED to drop procedure dba.forceDropTableList >>>' type   info to client ;
    else
        message '<<< Dropped procedure dba.forceDropTableList >>>' type info to client   ;
    end if ;
end if ;

create procedure dba.forceDropTableList(in @tablenamelist clob  ) ON EXCEPTION RESUME
BEGIN
	...
	do
		...
		while @sc != 0 loop
			IF (SELECT count(*) FROM SYSTAB WHERE table_name=@tname AND creator=(select user_id from SYSUSER where user_name=@towner) and table_type_str like 'BASE' ) > 0 then
				...
				set @sql='DROP TABLE '||@towner||'.'||@tname||';';
				execute immediate @sql;
				set @sc=sqlcode;
...
if (object_id('dba.forceDropTableList') is not   null)   then
   message '<<< Created procedure dba.forceDropTableList>>>' type info to client ;
   grant execute on dba.forceDropTableList to public ;
else
   message '<<< FAILED to create procedure dba.forceDropTableList >>>'   type info to client ;
end if ;
