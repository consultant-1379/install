if (object_id('dba.dropIdle_dcpublic_dcbo_Connections') is not   null)   then
    drop    procedure dba.dropIdle_dcpublic_dcbo_Connections ;

    if (object_id('dba.dropIdle_dcpublic_dcbo_Connections') is not null) then
        message '<<< FAILED to drop procedure dba.dropIdle_dcpublic_dcbo_Connections >>>' type   info to client ;
    else
        message '<<< Dropped procedure dba.dropIdle_dcpublic_dcbo_Connections >>>' type info to client   ;
    end if ;
end if ;

create procedure dba.dropIdle_dcpublic_dcbo_Connections()
begin
	...
	for loop1 as    curs1 cursor for
		select ConnHandle, DATEDIFF(mi,LastReqTime,now()) as idletime, Userid, IQconnID from  sp_iqconnection() where Userid in ('dcpublic','dcbo')
	do
...
if (object_id('dba.dropIdle_dcpublic_dcbo_Connections') is not   null)   then
   message '<<< Created procedure dba.dropIdle_dcpublic_dcbo_Connections>>>' type info to client ;
   grant execute on dba.dropIdle_dcpublic_dcbo_Connections to public ;
else
   message '<<< FAILED to create procedure dba.dropIdle_dcpublic_dcbo_Connections >>>'   type info to client ;
end if ;
